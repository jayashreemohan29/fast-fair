...
class btree{
	...
	public:
		...
		bool btree_insert(entry_key_t, char*);
		void btree_insert_internal(char *, entry_key_t, char *, uint32_t);
		bool btree_delete(entry_key_t);
...
bool remove(btree* bt, entry_key_t key, bool only_rebalance = false, bool with_lock = true) {
	...
	pid_t pid = fork();
        ...
        // child process
        if (pid == 0){
           	//cout << "\nChild process returning without inserting key \n" << key << endl;
                cout << "\n Child process returned without deleting split key" << endl;
                return true;
        }
         ...
         // parent process continue
			...
			page *left_sibling;
			bt->btree_delete_internal(key, (char *)this, hdr.level + 1, &deleted_key_from_parent, &is_leftmost_node, &left_sibling);
...
page *store (btree* bt, char* left, entry_key_t key, char* right, bool flush, page *invalid_sibling = NULL) {
	...
	if(num_entries < cardinality - 1) {
		...
		// child process
		if (pid == 0){
			// We return either this page or sibling and not null. Insert is retried, only if null is returned.
			//cout << "\nChild process returning without inserting key \n" << key << endl;
			cout << "\n Child process returned without updating split key" << endl;
			return ret;
		}
		...
		// Set a new root or insert the split key to the parent
			if(bt->root == (char *)this) { // only one node can update the root ptr
				//cout << "\nStep 2 of split - update root " << split_key << "\n" << endl;
				page* new_root = new page((page*)this, split_key, sibling, hdr.level + 1);
				bt->setNewRoot((char *)new_root);
			}
			else {
				//cout << "\nStep 2 of split - update internal " << split_key <<"\n" << endl;
...
bool btree::btree_insert(entry_key_t key, char* right){ //need to be string
	page* p = (page*)root;
...
(char *left, entry_key_t key, char *right, uint32_t level) {
	...
	//cout << "Return due to incorrect level" << endl;
	return;
	}
...
bool btree::btree_delete(entry_key_t key) {
	page* p = (page*)root;
...
bool *is_leftmost_node, page **left_sibling) {
	...
	//cout << "\nDeleting key at internal node " << key << endl;
	page *p = (page *)this->root;
